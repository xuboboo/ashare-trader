...
export const config = {
  ...
  /** 两笔“新仓”之间的最短间隔（毫秒，默认 90 秒）：只为拆掉“同一分钟机器人式齐买”的观感，
   *  不为了“看着像人”而错过几分钟内的真实行情（动量票入场时机本身就是 alpha）。
   *  真正防“无脑冲”靠的是逐笔新鲜确认 + 现金闸 + K + 每日开仓上限，不是这道时间。 */
  minOpenGapMs: num("MIN_OPEN_GAP_MS", 90_000),
