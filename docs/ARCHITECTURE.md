...
设计目标：把 jev-trader 里**与区块链无关**的那套骨架（可插拔模型、单事件在途循环、
影子撮合、含成本会计、SSE 仪表盘）完整保留，把与链机制绑死的部分换成 A 股规则。
...
| 时刻 | 做什么 | 不满足什么就不做 |
| --- | --- | --- |
| 09:05 | 拉涨停池算情绪、调 LLM 出当日 `allowOpen` 与 veto 名单 | 非交易日 |
| 09:30-10:00 | 持仓退出：到点清仓 > 高开减半 > 跌破止损 > 跌破分时均线 | 行情不新鲜、无可卖数量 |
| 14:40-14:57 | 全池打分 → 闸门 → top-K 建议单 | 闸门关、额度用完、行情不新鲜 |

其余时段只做心跳 + 影子撮合 + 盯市。`POST /scan` 是第四个入口：允许在收盘后强制跑一次
