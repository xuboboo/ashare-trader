...
一个把 [jev-trader](https://github.com/jarrodwatts/jev-trader) 的骨架搬到 A 股的项目：
可插拔的决策模型 + 单事件在途的循环 + 影子撮合记账 + SSE 实时仪表盘。
**决策节奏从"每 300ms 一次"改成"每个交易日三次"**：09:05 盘前闸门、09:30-10:00 持仓退出、14:40-14:57 尾盘选股。
...
| 盈利来源 | post-only 挂单吃价差 | 隔夜动量（**回测证明不成立**） |
| 下单 | 链上 `batchUpdate` 自动 | 人工在券商 App 执行 |
| 持仓 | 可多可空、随时平 | 只能做多、T+1 才能卖 |
| 成本 | gas + taker fee，maker 反而返佣 | 往返 11.6bp，且**降不下去** |
| 模型 | TypeSafe Jev 出概率 | 规则打分出概率；LLM 只做日频否决 |

A 股个股做不了链上那套的根因是三条叠加：T+1、涨跌停与停牌、程序化交易报备
（现行高频认定线为单账户每秒申报/撤单合计 ≥300 笔或单日 ≥2 万笔；2026 年疯传的
"降到每秒 15 笔"不在任何条文中，已辟谣）。第四条最致命：**5 元最低佣金 + 印花税**
把零售往返成本钉在 11.6bp 附近 —— 加大单笔金额也没用，因为超过 20 万后成本是比例费主导。
