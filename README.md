...
## 数据级别与延迟（实测 2026-09-20，`bun run scripts/probe-latency.ts`）

**级别：L1 快照，不是 L2。** 有最新价、五档、当日累计量额、分时均价、量比、涨跌停价；
没有十档、逐笔委托、逐笔成交。交易所 L1 本身就是 **3 秒一个切片**，这是物理下限，
与本机网络无关。别拿这套数据想盘中挂撤单 / 抢单价差。

三层延迟：

| 层 | 实测 | 说明 |
| --- | --- | --- |
| 传输往返 | 腾讯快照 43ms（最差 176）、东财榜单 25ms、涨停池 29ms、新浪 8ms | 本机到接口 |
| 引擎一轮 | 中位 58ms（最差 967ms 是首轮，含拉日历与日线），轮询 3000ms | 指数 + 300 支快照 + 打分 + 撮合 |
| 数据新鲜度 | 盘中才会接近真值；收盘后快照时间戳就是上一交易日 | `quotes.ageSec`，阈值 `QUOTE_STALE_SEC=30` |

两个由新鲜度/数据级别推导出的硬规则（不遵守就不会出单）：

1. **行情老化就什么都别做**：盘中 `ageSec > QUOTE_STALE_SEC` 时，本轮不选股、不出退出单、
   也不做影子撮合（`engine.roundInner` 里的 `usable`），仪表盘顶部亮黄色横幅并报出具体秒数。
   拿隔夜价当成活价下单，是这类系统最容易犯的错。
2. **纸面撮合不得美化自己的成交**（`orders.tryPaperFill`）：
   - 成交价用**下一轮观测到的现价** + 滑点，不是下单那一刻的参考价 —— 3 秒里跑掉的价必须付；
   - 判“能不能成交”只看挂单之后观察到的极值（`seenLow`/`seenHigh`，由 `updateResting` 逐轮累加），
     **不看全天累计高低点** —— 09:31 的最低价不能让你 14:45 挂的单“成交”；
   - 买入价被 `limitHigh` 钳住、卖出价被 `limitLow` 钳住：价格已经跑远了就不算追价成功；
   - 一字涨停买不进、一字跌停卖不出。

仍然存在的口径差异（往盘中推进之前得先解）：日频回测把“次日 10:00 前清仓”记为**收盘价**，
而实时路径是下一轮观测价 ± 滑点；另外回测的止损用当日最低价判定（人工执行下合理，
但不是逐笔可验证的）。要消除这个差异，需要分钟线，或接券商接口的委托回报。
